...
const SubmitPage = () => {
   ...
   const handleSubmit = async (e) => {
    ...
    try {
        ...
        const bookingPromises = ticketEntries.map((entry, index) => {
            const bookingData = {
                user_id: userId || null, // User ID from local storage or null if not found
                schedule_id: scheduleId,
                ticket_type: ticketType,
                seat_no: bookedSeatNum[index],
                date: currentDate,
        ...
        if (errorMessages.length > 0) {
            setError(errorMessages.join(', ')); // Set the error message to state
            setSuccessMessages([]); // Clear any success messages
        } else {
            setSuccessMessages(successMessages);
            setIsBookingComplete(true);
            setTicketEntries([]); // Clear the form fields upon success
        }
    } catch (error) {
        console.error('Error during booking:', error);
        setError('An error occurred while booking the flight. Please check your input and try again.'); // Set error message on catch
        setSuccessMessages([]); // Clear success messages if there’s an error
    } finally {
        setLoading(false);
    }
};
