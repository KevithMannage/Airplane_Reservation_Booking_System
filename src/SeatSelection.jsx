...
const SeatSelection = ({ onSeatSelect }) => {
  ...
  const [ticketType, setTicketType] = useState('Economy'); // Default ticket type
  const scheduleId = 6; // Set your schedule ID here
  ...
  useEffect(() => {
    const fetchSeats = async () => {
      try {
        ...
        console.log("Fetched seats:", data); // Log the fetched seats
        setSeats(data); // Assuming data is an array of seat objects
      } catch (error) {
  ...
  console.log("Number of seats to display:", seats.length); // Log the number of seats
